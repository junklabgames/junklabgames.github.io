...
export default function News({ data }) {
  return (
    <section className="text-center max-w-screen-lg px-4 md:px-0 mx-auto mt-24">
      ...
      <p className="mb-4">The latest and greatest news and development progress from Junklab Games. Follow our social accounts to keep up to date with all the latest updates.</p>
        ...
        {data.allMarkdownRemark.edges.map((post) => {
          ...
          return (
            <div className="w-article">
              <Post title={title} date={date} key={`${date}__${title}`} path={path} featuredImage={featuredImage} />
            </div>
          );
        })}
      </article>
